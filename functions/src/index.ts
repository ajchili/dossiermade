...
exports.convertImageToBeProgressive = functions.runWith({
  timeoutSeconds: 300,
  memory: "1GB"
  ...
  if (fileBucket.length === 0 || filePath.length === 0 || contentType.length === 0) {
  ...
  const thumbFilePath = path.join(path.dirname(filePath), "thumb", thumbFileName);
